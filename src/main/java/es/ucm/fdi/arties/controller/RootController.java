...
public class RootController {
    ...
    public String index(Model model, HttpSession session) {
        putComundDataInModel(model, session);
         ...
         List<Item> allItems = dbItems.getItemList(em);
         if(allItems.size() >=2)
             model.addAttribute("items", allItems.subList(0, 2));
         else
             model.addAttribute("items", allItems);
 
         List<Lesson> allLessons = dbLessons.getAllLessons(em);
         if(allLessons.size() >=4)
             model.addAttribute("lessons", allLessons.subList(0, 4));
         else
         ...
         if(coursesList.size() >= 5)
             model.addAttribute("freeCourses", coursesList.subList(0, 5));
         else
             model.addAttribute("freeCourses", new ArrayList<Course>());
