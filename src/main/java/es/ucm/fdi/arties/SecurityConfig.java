	...
	protected void configure(HttpSecurity http) throws Exception {
	    http
			.csrf()
				...
				.and()
	        .authorizeRequests()
			.antMatchers("/css/**", "/js/**", "/img/**", "/", "/error", "/signup", "/courses", "/seeCourse", "/items/listToLoan", "/lessons/lessonsToBook").permitAll()
				.antMatchers("/api/**").permitAll()            // <-- public api access
				.antMatchers("/admin/**", "/existeUsuario", "/getUser").hasRole("ADMIN")	   // <-- administration
				.antMatchers("/settings", "/user/deleteUser", "/user/addStaff", "/addCourse", "/createCourse", "/modifyCourseImg").hasRole("ADMIN")
				//items admin
				...
				.antMatchers("/lessons/addNewLesson").hasRole("ADMIN")
				...
				.antMatchers("/subscriptions", "/lessons", "/items/myItems","/items/loanItem", "/items/makeLoan", "items/availableItemDay", "/modifyUser", "/getConversation", "/getStaffList", "/lessons/myBookingLessons").hasAnyRole("USER", "ADMIN", "STAFF")
